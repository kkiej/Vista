...
namespace Vista
{
    ...
    public class VistaVolumetricFogSettings
    {
        ...
        [Tooltip("历史失效的亮度死区下端：相对亮度变化小于这个值时**完全不降权**。\n\n"
               ...
               + "0.55 是两条实测曲线夹出来的，不是调出来的：\n"
               + "· 去噪侧（判据⑯c 扫描）：下端 → 实际保住的历史权重 w_eff/w\n"
               + "    0.10→0.527  0.25→0.643  0.40→0.834  0.55→0.984  1.00(关闭)→1.021\n"
               + "  判据⑯a-1 要求 R ≥ R_white×0.75 = 5.35，实测 0.40→3.02 不过、0.55→6.00 过。\n"
               + "  所以 0.40 是纯亏损：付了判别力，仍然过不了去噪门。\n"
               + "· 拖影侧（判据⑰）：一次 +50% 光照阶跃的 T50\n"
               + "    0.25→9.3 帧  0.55→15.4 帧  1.00(关闭)→15.9 帧\n"
               + "  0.55 与「关闭」差 0.5 帧 < ⑰ 自己的 1.6 帧可分辨地板 ⇒ 在这个量级上等价。\n\n"
               + "★ 0.55 不是「把死区调好了」，是承认它在当前形态下不工作，要读懂这一条：\n"
               + "rel 是**逐 froxel** 算的（FroxelVolume.hlsl:598），而逐 froxel 只做 1 tap 硬阴影，"
               + "骑在阴影边界上的格子每帧在全亮/全暗之间整跳 ⇒ 那里的 rel 直接冲到 1。"
               + "于是死区优先关掉的正是**阴影边界上**的时间累积 —— 而那里恰好是累积唯一必要的地方，"
               + "它与需求反相关。根子是信息不够：这里只有「本帧」「历史」两个标量，"
               + "没有屏幕空间 TAA 那样的邻域统计去区分「这次变化是噪声还是信号」，"
               + "所以任何标量阈值都分不开两个重叠的分布。\n"
               + "真正的修法是把 1 tap 换成 PCF（砍掉 rel 的尖峰），或改成带 froxel 邻域方差的 clamp。"
               + "在那之前，0.55 买到的是「去噪全额 + 拖影等于没有死区」。")]
        [Range(0f, 1f)] public float luminanceRejectStart = 0.55f;
