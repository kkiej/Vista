...
//
//  x 小时改用 (1 - e^{-x})/x = 1 - x/2 + x²/6 - … 的截断展开：展开式整个不做
//  除法，σ 兜底到 1e-30 的大气顶附近也不再放大误差。
//  逐通道选择：RGB 三个方向的 σ 差 6 倍，同一步长可能一个通道相消一个不相消。
//
//  ---- 阈值 x* 是解出来的，不是试出来的（#25 把它从 1e-4 换成下面这个数）----
//  两支的固有误差朝相反方向发散，而且各自都有闭式：
//    exact 支：1 − e^{−x} 的绝对误差是 exp 自身的约 1 ulp（ε = 2⁻²³ = 1.192e-7），
//              而结果本身 ≈ x ⇒ **相对**误差 ≈ ε/x，x 越小越糟；
//    series 支：首个被丢掉的项是 x³/24 ⇒ 相对误差 ≈ x³/24，x 越大越糟。
//  令两者相等：ε/x = x³/24 ⇒ x⁴ = 24ε ⇒ x* = (24·2⁻²³)^¼ = 4.1127e-2。
//  这一点上两支的误差都等于 ε/x* = 2.90e-6，而那正是整个函数的**最坏相对误差**
//  —— 往任一侧走，被选中的那一支的误差都在下降，所以 x* 是极小极大点。
//
//  旧的 1e-4 比 x* 低了两个半数量级。代价不是理论上的：x ∈ [1e-4, 4.11e-2]
//  这一整段里代码故意走了误差更大的那一支，而生产档（自检 A 档：近 0.3 m /
//  远 64 m / 64 片、τ = 3）实测的 x ∈ [1.336e-3, 2.411e-1]，**最短一段**
//  x = 1.336e-3 正落在里面 —— 它此前的固有相对误差是 ε/1.336e-3 = 8.92e-5，
//  换完之后走级数支，是 x³/24 = 9.9e-11。换完后全档最坏的单段误差降到 2.87e-6
//  （由 x* 处的极小极大值封顶）。
//
//  ---- 诚实地说：这次替换没有任何判据能把它验绿 ----
//  froxel 深度积分自检的门是 2e-3（fp16 存储地板 ×2.05）。就算把「每一段都取
//  最坏」当成整柱的上界，换前的 8.92e-5 也只有门的 1/22，落在门的分辨力之下；
//  何况单段误差是按该段对整柱的贡献权重摊进去的，最短的那一段恰恰权重最小，
//  实际影响远小于这个上界 —— 所以门两边读数不变是预期之内，不是巧合。
//  也就是说它是**推导上的**改进；判据能保证的只有两件事：全套门原地不动地
//  继续绿（没变坏），以及两支都仍然被真实档位走到（自检里那一格跨档覆盖）。
//  把它写成「实测提升」会是一句假话。
//
...
//  再顺着三线性插值蔓延到全屏。
//
//  常数只需满足一件事：小到「一旦 max() 真的生效，x 必然 ≤ x*，也就是
//  这个 exact 值一定会被 lerp 丢掉」。1e-30 对任何 dt ≤ 4.1e28 km 都成立
//  （阈值抬到 x* 之后这条余量还宽了两个半数量级），
//  所以它不是一个调出来的阈值 —— 它对**当前返回非 NaN 的每一个输入
//  都逐位不改变结果**，只把 NaN 那一格填成一个会被丢弃的有限数。
// ----------------------------------------------------------------------------

// x* = (24·2⁻²³)^¼，推导见上。写成具名常量而不是散在函数里的字面量：
// 自检里有两处「逐字照抄」的副本（都叫 k_SeriesThreshold —— 一个在
// VistaFroxelIntegrationSelfTest，一个在 VistaVolumetricFogState），
// 有个名字它们才能指着同一个东西对齐，而不是三处各写一遍同一个魔数。
// 前者还会把自己那份和 (24·ulp)^¼ 当场算一遍比对，走歧了会打叉。
#define VISTA_SEGMENT_SERIES_X 4.1127e-2
...
float3 VistaSegmentIntegral(float3 source, float3 extinction, float dt)
{
    ...
    float3 useSeries = step(x, VISTA_SEGMENT_SERIES_X);   // x <= x* 时取 1
