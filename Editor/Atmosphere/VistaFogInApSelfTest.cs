...
namespace Vista.Editor
{
    ...
    public static partial class VistaAtmosphereSelfTest
    {
        ...
        struct FogView
        {
            ...
            ///
            /// ⚠ 早先在 CHANGELOG 里写过「#19 近层 froxel 体落地后这两个字段必须删掉」，
            /// **那条待办的前提是错的，已撤销**（#19 收尾时发现）。理由：本视角的雾拐点在
            /// **877 m**，而近层体覆盖的是 **[0, 61.374 m]**（默认档的接手点）—— 差 14 倍。
            /// 拐点无论如何都落在 AP LUT 的区间里，近层体接不走它。
            /// 更根本的是档 D **按定义**没有近层体（`Mode.AerialPerspective` 是移动端主线，
            /// 无阴影查询、无新纹理），所以只要档 D 还发布，这两个基线就一直是
            /// 档 D 表达能力上限的诚实陈述，而不是等着被拆掉的脚手架。
            /// 当时的混淆是把「近层体接走近段的雾」读成了「近层体接走视角③ 的雾拐点」。</summary>
        ...
        static readonly FogView[] k_FogViews =
        {
            ...
            //   （注：这里原先写着「这正是 #19 存在的理由」——**不准确**。#19 的近层体
            //    覆盖 [0, 61 m]，接不走 877 m 的拐点；能把这个拐点从表里拿走的是
            //    逐像素解析闭式解那条路，不是近层体。见 capacityCeiling 字段的注释。）
