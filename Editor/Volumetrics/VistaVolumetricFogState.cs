        ...
        static void Run(StringBuilder sb)
        {
            ...
            // ================================================================ #22a 时间重投影
            {
                ...
                else
                {
                    ...
                    // ⑮b 已由「门」降级为「读数」（#27 第二阶段）。
                    //
                    // 它此前每次运行都印 ✘（0.2500 > 0.809 不成立），但它的**处方在上一步
                    // 就已经撤回**了 —— 一个不再主张任何东西的判据还在红，训练出来的是
                    // 「忽略 ✘」。报表上最贵的东西是 ✘ 的可信度，一个长期红着而且我们
                    // 自己知道它红得没有意义的门，是在花掉这份可信度。
                    //
                    // 降级不是把欠账抹掉：真正的门要等判据⑯（同一格丢历史之后噪声涨多少）
                    // 实现之后重写，欠条记在 CHANGELOG 的「待办」里 —— 那儿不会被误读成
                    // 「今天有个东西坏了」。
                    if (jitterOn)
                    {
                        sb.AppendLine($"ⓘ 读数⑮b（已降级，不再是门）抖动自己制造的散布 max = {Sci(spread)}，"
                                    + $"当前死区下端 {deadStart:F4}（差 {Sci(deadStart - spread)}，带符号）");
                        sb.AppendLine("  ⓘ 散布量的是**同一个 froxel 两次独立抖动抽样**的相对亮度差的 max，"
                                    + "不是「历史与本帧之差」—— 后者混着相机运动与场景变化。");
                        sb.AppendLine("  ⓘ 为什么不再是门：**统计量选错了**。死区规则问的是"
                                    + "「有多少格的 rel 超过 luminanceRejectStart」，而这一格给的是"
                                    + "16384 格上的 max —— max 必然被 1/16384 的极端值定住，"
                                    + "对「多数格子在哪儿」是结构性失明的。"
                                    + "同配置 24 次运行它的跨度是 ×2.7（11 次饱和在 1.0），"
                                    + "而下面那张 rel 直方图的越线占比两次只差 2%。"
                                    + "**该往哪儿摆由直方图回答，不由这一格回答。**");
                        sb.AppendLine("  ⓘ 曾经写在这里的病因「max 被 lA≈0 的黑格子占住、分母撞上 1e-6 地板」"
                                    + "**已被下面的亮度分层表证伪**：最暗的桶也在 0.125 以上，"
                                    + "离那个地板还有六个数量级。这个 max 量到的是**真实**的相对亮度差，"
                                    + "它没测错，只是它测的东西不该被当成死区下端。");
                        if (raw[k_SlotReprojJitterSpread] >= (uint)k_ReprojJitterScale)
                            sb.AppendLine("  ⚠ 本帧读数**精确饱和在 1.000**（saturate 的上限）。"
                                        ...
                                        + "也可能是若干格同时贴顶。这一格此时只剩「存在某格跨过了明暗边界」这一条信息。");
                        sb.AppendLine($"  ⓘ 保留它不删的理由：它有 24 次历史读数，跨配置的时间序列要能对得上；"
                                    + $"并且它是「{k_JitterDeadbandSymptom}」这条症状目前唯一的线上探针 ——"
                                    + "只是它证明不了这条症状**发生了**（毁掉的若是一格辐亮度≈0 的雾，画面上没有代价）。");
                    ...
                    {
                        sb.AppendLine("ⓘ 读数⑮b（已降级，不再是门）：jitterMode = Off ⇒ 抖动幅度 0 ⇒ "
                                    + $"两次抽样逐位相同 ⇒ 散布恒为 0（实测 {Sci(spread)}）。"
                                    + "此档下这一格与下面的 rel 直方图**都**没有区分力，点名它。");
                    }
            ...
            sb.AppendLine("ⓘ 未覆盖（推迟到 #27）：判据⑯ 收敛性 —— 「累积真的降了噪、且没有引入偏差」。"
                        ...
                        + "而它要打败的噪声由上面读数⑮b 那个 JITTER_SPREAD 量出来了。\n"
