Shader "Hidden/Vista/FroxelDebug"
{
    ...
    //
    //  ---------------------------------------------------------- 为什么需要它
    //  #21 交付的是一张 3D 表（累积 in-scatter 与 1 − T）。它落地时**消费它的合成还没写**
    //  （合成在 #25 落地），没有这个视图，#21 交付的东西在画面上完全不存在，
    //  唯一的证据是日志里的数字 —— 那等于把一整条写入路径交付成
    //  「一段永远不会被发现写错的代码」。
    //
    //  合成落地**之后**它仍然有用，所以它是永久资产而不是脚手架：
    ...
    //  —— #27 的夜景布景上它已经兑现过一次：地面一片灰、看不出锥，
    //  把局部灯 intensity 置 0 再拍一张同参数图，地面变纯黑，
    //  才把「表里没有光柱」与「光柱被背景淹没」分开。
    //  单独一张调试图回答不了「这里面有没有 X」，要有一张把 X 关掉的对照图。
    //
    //  ---------------------------------------------------------- 为什么整屏替换而不是叠加
